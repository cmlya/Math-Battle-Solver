...
def closest_coord(query_coord, coords):
    return min(
        math.hypot(c[0] - query_coord[0], c[1] - query_coord[1])
        for c in coords
    )
...
def imagesearcharea(template, x1, y1, x2, y2, precision=0.8, im=None):
    if im is None:
    ...
    if template is None:
        raise FileNotFoundError('Image file not found: {}'.format(template))

    res = cv2.matchTemplate(img_gray, template, cv2.TM_CCOEFF_NORMED)
    loc = np.where(res >= precision)
    coords_unfiltered = list(zip(*loc[::-1]))
    coords = []
    for coord in coords_unfiltered:
        if len(coords) == 0 or closest_coord(coord, coords) > 3 ** 2:
            coords.append(coord)
    return [(x1 + coord[0], y1 + coord[1]) for coord in coords]
...
def imagesearch_v2(template, precision=0.8):
    with mss.mss() as sct:
...
def imagesearch(template, precision=0.8):
    with mss.mss() as sct:
        im = sct.grab(sct.monitors[0])
        if is_retina:
            im.thumbnail((round(im.size[0] * 0.5), round(im.size[1] * 0.5)))
        ...
        img_rgb = np.array(im)
        # img_rgb = cv2.resize(img_rgb, (img_rgb.shape[0] // 3, img_rgb.shape[1] // 3))
        img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
        # template = cv2.resize(template, (template.shape[0] // 3, template.shape[1] // 3))
        if template is None:
            raise FileNotFoundError('Image file not found: {}'.format(template))
        # template.shape[::-1]
        ...
        #     print(len(coords))
        #     for coord in coords:
        #         cv2.rectangle(img_rgb, coord, (coord[0]+18,coord[1]+85), (0,0,255), 2)
        #     cv2.imwrite("debug_output.png", img_rgb)
        return coords
...
def imagesearch_numLoop(image, timesample, max_samples, precision=0.8):
    pos = imagesearch(image, precision)
    count = 0
    while pos[0] == -1:
        print(image + " not found, waiting")
...
def imagesearch_count(image, precision=0.9):
    with mss.mss() as sct:
        ...
        for pt in zip(*loc[::-1]):  # Swap columns and rows
            cv2.rectangle(img_rgb, pt, (pt[0] + w, pt[1] + h), (0, 0, 255), 2)  # Uncomment to draw boxes around
            # found occurrences
            count = count + 1
...
def imagesearch_from_folder(path, precision):
    ...
    files = [f for f in os.listdir(path) if
             os.path.isfile(os.path.join(path, f)) and os.path.splitext(f)[1].lower() in valid_images]
    for file in files:
